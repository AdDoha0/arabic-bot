...
;
        ...
        let request = self.create_completion_request(prompt, &self.context).await?;
...
impl CreatePractice {
    ...
    pub async fn get_more_practice(&mut self) -> Result<String, Box<dyn std::error::Error>> {
        let fixed_prompt = "еще задания. НЕ ПИШИ НИЧЕГО ДРУГОГО КРОМЕ ЗАДАНИЙ. не нужно в начале писать клише";
